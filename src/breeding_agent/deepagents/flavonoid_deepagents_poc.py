...
def build_deepagents_trace(agent_result: dict[str, object]) -> list[dict[str, object]]:
    ...
    agent_outputs = _as_list(agent_result.get("agent_outputs"))
    ...
    trace: list[dict[str, object]] = [
        {
            ...
            "input_summary": _context_summary(agent_context),
        ...
        },
        {
            ...
            "output_summary": f"Aggregated {len(candidate_rows)} fixed target genes.",
        ...
        },
        {
            ...
            "output_summary": _context_summary(agent_context),
            "evidence_used": [
...
def render_deepagents_summary(
    *,
    ...
    variant_calling_dir = agent_result.get("variant_calling_dir")
    agent_names = [
        row["agent_name"]
        for row in decision_rows
        if row.get("agent_name") and row["agent_name"].endswith("_agent")
    ]
    lines = [
        ...
        f"- variant evidence 接入: {'yes' if variant_calling_dir else 'no'}",
...
def _context_summary(context: dict[str, object]) -> str:
    return (
        ...
        f"variant_rows={len(_as_list(context.get('variant_evidence')))}"
    )
